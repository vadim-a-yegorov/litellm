...
class Router:
    ...
    async def _aanthropic_messages_streaming_iterator(
        ...
        async def stream_with_fallbacks() -> AsyncGenerator[bytes, None]:
            ...
            # stream ends without ever producing content.
            has_generated_content = False  # rebind-ok: set once real content is seen
            ...
            try:
                async for chunk in source_iterator:
                    if is_anthropic_content_delta_chunk(chunk) and not has_generated_content:
                        has_generated_content = True  # rebind-ok: real content seen in this chunk
                    ...
                    if has_generated_content or not (status_code == 429 or status_code >= 500):
                        for buffered_chunk in buffered_lifecycle_chunks:
                            yield buffered_chunk
                        buffered_lifecycle_chunks = ()
                        yield chunk
                ...
                # A MidStreamFallbackError raised directly by the source
                # iterator (the completion-bridge path's CustomStreamWrapper,
                # e.g. on a transport drop) carries its own pre_first_chunk
                # bookkeeping - respect it the same way a detected SSE error
                # event is gated, so a fallback is never appended after real
                # content already reached the client on either path.
                if has_generated_content or not e.is_pre_first_chunk:
                    for buffered_chunk in buffered_lifecycle_chunks:
                        yield buffered_chunk
                    if e.original_exception is not None:
                        raise e.original_exception from e
