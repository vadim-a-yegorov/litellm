...
def _anthropic_content_block_start_and_delta(
    block: Mapping[str, object],
) -> tuple[Mapping[str, object], Mapping[str, object] | None]:
    """
    ``(content_block_start.content_block, content_block_delta.delta)`` for one
    Anthropic response content block, or ``(content_block, None)`` when the
    block type has no streaming delta (redacted_thinking is sent complete).
    """
    match block.get("type"):
        ...
        case "redacted_thinking":
            return ({"type": "redacted_thinking", "data": block.get("data")}, None)  # mutable-ok: one-shot JSON payload
        case _:
            return (
...
def anthropic_messages_response_as_sse_events(response: AnthropicMessagesResponse) -> tuple[bytes, ...]:
    ...
    message_start_payload: Final = {  # mutable-ok: one-shot JSON payload, never mutated after construction
        "type": "message_start",
        "message": {**response, "content": []},  # mutable-ok: one-shot JSON payload
    }
...
def _anthropic_content_block_events(index: int, block: Mapping[str, object]) -> tuple[bytes, ...]:
    start_block, delta = _anthropic_content_block_start_and_delta(block)
    delta_payload: Final = {  # mutable-ok: one-shot payload
    ...
    delta_events: Final = (_sse_event("content_block_delta", delta_payload),) if delta is not None else ()
    return (
